...
fn g2p(@builtin(global_invocation_id) id: vec3<u32>) {
    if (id.x < numParticles) {
       ...
       let torusParams = vec2<f32>(2.0, 0.5)  * boundSize * .1;
       let distanceBox = sdBox(dist, boxParams);
        let distanceTorus = sdTorus(dist, torusParams);

        
        let distance  = distanceTorus;
        let params_shape = torusParams;
        let epsilon = 0.001;
       
        let gradient = vec3<f32>(
            sdTorus(dist + vec3<f32>(epsilon, 0.0, 0.0), params_shape) - distance,
            sdTorus(dist + vec3<f32>(0.0, epsilon, 0.0), params_shape) - distance,
            sdTorus(dist + vec3<f32>(0.0, 0.0, epsilon), params_shape) - distance
        ) / epsilon;
        if (distance < 0.0) {
            particles[id.x].v += -distance * normalize(gradient) * 6.0;
        }
