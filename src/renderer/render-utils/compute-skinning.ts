...
import { GeometryLayout, GeometryLocationDesciption } from "../../geometry/geometry-layout.js";
...
function getSkinningShader(layout: Readonly<GeometryLayout>) {
  ...
  console.log("is16BitJoints",is16BitJoints)
  return wgsl`
...
function getAttributeSize(desc: GeometryLocationDesciption): number {
  if (!desc) { return 0; }
...
export class ComputeSkinningManager {
  ...
  constructor(public renderer: RendererBase) {
    
  }

  #getSkinningPipeline(layout: Readonly<GeometryLayout>): GPUComputePipeline {
    const key = layout.id;
    ...
    if (!pipeline) {
      const entries: GPUBindGroupLayoutEntry[] = [{
        ...
        binding: 2,
        buffer: { type: 'read-only-storage' }, // joint buffer
        visibility: GPUShaderStage.COMPUTE
      }, {
        binding: 3,
        buffer: { type: 'read-only-storage' }, // weight buffer
        visibility: GPUShaderStage.COMPUTE
      }, {
        binding: 4,
        buffer: { type: 'read-only-storage' }, // position buffer
        visibility: GPUShaderStage.COMPUTE
      }];

      const getStride = (location: number) => {
        const desc = layout.getLocationDesc(location);
        if (!desc) { return 0; }
        return desc.arrayStride / 4;
      };

      const constants: {[x: string]: number} = {
      ...
      if (layout.locationsUsed.has(AttributeLocation.normal)) {
        entries.push({
          binding: 5,
          buffer: { type: 'read-only-storage' }, // normal buffer
          visibility: GPUShaderStage.COMPUTE
        });
      ...
      if (layout.locationsUsed.has(AttributeLocation.tangent)) {
        entries.push({
          binding: 6,
          buffer: { type: 'read-only-storage' }, // tangent buffer
          visibility: GPUShaderStage.COMPUTE
        });
  ...
  #getSkinnedGeometry(geometry: RenderGeometry): SkinnedGeometry {
    ...
    if (!skinned) {
      ...
      let skinnedGeometryDesc: GeometryDescriptor = {
