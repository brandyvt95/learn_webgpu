...
@location(1) uv : vec2f
  ...
  let A = vec3 < f32 > (
    f32(pointss[baseIndex + 0u]),
    f32(pointss[baseIndex + 1u]),
    f32(pointss[baseIndex + 2u])
  );

  let B = vec3 < f32 > (
    f32(pointss[baseIndex + 3u]),
    f32(pointss[baseIndex + 4u]),
    f32(pointss[baseIndex + 5u])
  );
  let direction = B - A;
